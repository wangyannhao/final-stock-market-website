...
def ann_predict_long(tbl, predict_range=30):
    table = get_data_db(tbl)
    close = zip(*table)[1]
    close = np.asarray(map(float,close[0:len(close)-1]))
    result  = predict_ann_long(close, predict_range )
    return result
...
def svm_predict_long(tbl, predict_range):
    table = get_data_db(tbl)
    date = zip(*table)[0]
    date = date[0:len(date)-1]
    close = zip(*table)[1]
    close = np.asarray(map(float,close[0:len(close)-1]))
    result = predict_svm_long(close, predict_range)
    return result

def regression_predict_long(tbl, predict_range):
    table = get_data_db(tbl)
    date = zip(*table)[0]
    date = date[1:len(date)-1]
    close = zip(*table)[1]
